...
#define OSHMPI_SIGNAL_COMP(a, comp_op, b, ret /* OUT */, sat /* OUT */) do {       \
    switch (comp_op) {                                                             \
        case SHMEM_CMP_EQ:                                                         \
            ret = (a == b) ? 1 : 0; sat = a; break;                                \
        case SHMEM_CMP_NE:                                                         \
            ret = (a != b) ? 1 : 0; sat = a; break;                                \
        case SHMEM_CMP_GT:                                                         \
            ret = (a > b) ? 1 : 0; sat = a; break;                                 \
...
/* Similar to OSHMPI_WAIT_UNTIL, but set 'satisfied_value' to the result. */
#define OSHMPI_SIGNAL_WAIT_UNTIL(ivar, comp_op, comp_value, C_TYPE, MPI_TYPE, satisfied_value) do { \
    C_TYPE tmp_var;                                                                                 \
    MPI_Aint target_disp = -1;                                                                      \
    OSHMPI_ictx_t *ictx = NULL;                                                                     \
    OSHMPI_sobj_attr_t *sobj_attr = NULL;                                                           \
    ...
    while (1) {                                                                                     \
        OSHMPI_CALLMPI(MPI_Fetch_and_op(NULL, &tmp_var, MPI_TYPE,                                   \
                                        ...
                                        ictx->win));                                                \
        OSHMPI_CALLMPI(MPI_Win_flush_local(OSHMPI_global.team_world_my_pe, ictx->win));             \
        OSHMPI_SIGNAL_COMP(tmp_var, comp_op, comp_value, comp_ret, satisfied_value);                \
        if (comp_ret) break;                                                                        \
        OSHMPI_am_cb_progress();                                                                    \
        OSHMPI_progress_poll_mpi();                                                                 \
    }                                                                                               \
